...
You can use `query_events` to track the events that a specific package, module, or function emits. 
Using this function call is especially helpful when you want to track network activity. For example,
you can track the minting of SuiFrens Capys as they happen using the following cURL command:
...
See [Sui Events API](https://docs.sui.io/build/event_api) for more information.

The following example is a continuation of the example smart contract for events. If needed, you could
adjust the example to emit different kinds of events.
